...
public class ServiceMetier {
    ...
    public void rechercherClientParDenomination(String denomination, String ville) throws ServiceException {
        try {// 1. Obtenir la liste des Clients

            JsonObject clientsContainer = null;
            try {
    ...
    public void rechercherClientParNomPersonne(String nomPersonne, String ville) throws ServiceException {
        try {
            // 0. Récupérer la liste des personnes
            ...
            final JsonArray personneIdsArray = new JsonArray();
            jsonPersonnesListe.forEach(personne -> personneIdsArray.add(personne.getAsJsonObject().get("id")));

            JsonObject clientsContainer;
            try {
                clientsContainer = this.jsonHttpClient.post(
                        this.somClientUrl,
                        new JsonHttpClient.Parameter("SOM", "rechercherClientParPersonne"),
                        new JsonHttpClient.Parameter("personne-ids", personneIdsArray.toString()),
                        new JsonHttpClient.Parameter("ville", ville)
                );
            }
            catch (ServiceIOException ex) {
